...
function timer_loop()
    ...
    obs.obs_frontend_set_transition_duration(1000)
...
function toggle_script(state)
    ...
    if enabled then
        scene_idx = 0
        timer_loop()
        obs.timer_add(timer_loop, 5000) -- 4000ms espera + 1000ms Fade
    end
end

function script_load(settings)
    obs.obs_hotkey_register_frontend("hk_sep_8", "Activar Bucle 8", function(p) if p then toggle_script(not enabled) end end)
end